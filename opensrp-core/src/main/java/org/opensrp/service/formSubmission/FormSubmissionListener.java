...
public class FormSubmissionListener {
	...
	public FormSubmissionListener(FormSubmissionService formSubmissionService, FormSubmissionProcessor fsp,
		...
		this.fsp = fsp;
		this.configService.registerAppStateToken(AllConstants.Config.FORM_ENTITY_PARSER_LAST_SYNCED_FORM_SUBMISSION, 0,
		...
		try {
			...
			for (FormSubmission submission : formSubmissions) {
				try {
					...
					if (submission.getField("no_client_event") == null
					        || submission.getField("no_client_event").contains("false")) {
						fsp.processFormSubmission(submission);
					}
					
					configService.updateAppStateToken(AllConstants.Config.FORM_ENTITY_PARSER_LAST_SYNCED_FORM_SUBMISSION,
					    submission.serverVersion());
				}
				catch (Exception e) {
					e.printStackTrace();
					errorTraceService.addError(new ErrorTrace(DateTime.now(), "FormSubmissionProcessor", this.getClass()
	...
	private long getVersion() {
		AppStateToken token = configService
		        .getAppStateTokenByName(AllConstants.Config.FORM_ENTITY_PARSER_LAST_SYNCED_FORM_SUBMISSION);
