...
public class AllActionsIntegrationTest {
	...
	public void shouldReturnAlertActionsBasedOnCaseIdAndScheduleNameAndTimeStamp() throws Exception {
		...
		assertEquals(asList(firstAction, secondAction, thirdAction), allActions
				.findByCaseIdScheduleAndTimeStamp("Case X", SCHEDULE_NAME, new DateTime(0),
						new DateTime(thirdAction.getTimeStamp())));
		assertEquals(asList(secondAction, thirdAction), allActions
				.findByCaseIdScheduleAndTimeStamp("Case X", SCHEDULE_NAME, new DateTime(firstAction.getTimeStamp() + 1),
						new DateTime(thirdAction.getTimeStamp())));
		assertEquals(asList(thirdAction), allActions
				.findByCaseIdScheduleAndTimeStamp("Case X", SCHEDULE_NAME, new DateTime(secondAction.getTimeStamp() + 1),
						new DateTime(thirdAction.getTimeStamp())));
		assertEquals(0, allActions
				.findByCaseIdScheduleAndTimeStamp("Case X", SCHEDULE_NAME, new DateTime(thirdAction.getTimeStamp() + 1),
						new DateTime(thirdAction.getTimeStamp() + 2)).size());
	}
	...
	private ActionData alert() {
		return ActionData
				.createAlert(mother.value(), SCHEDULE_NAME, VISIT_CODE, normal, DateTime.now(), DateTime.now().plusDays(3));
	}

	private ActionData alert(String schedule, String milestone) {
		return ActionData
				.createAlert(mother.value(), schedule, milestone, normal, DateTime.now(), DateTime.now().plusDays(3));
	}
}
