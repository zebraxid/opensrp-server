...
import org.opensrp.register.mcare.service.ChildRegistrationService;
...
public class RegisterController {
	...
	private ChildRegistrationService childRegisterService;
	...
	@Autowired
	public RegisterController(ChildRegistrationService childRegisterService, ChildRegisterMapper childRegisterMapper,
	    MultimediaRegisterService multimediaRegisterService, DataCountService dataCountService) {
	...
	public ResponseEntity<ChildRegisterDTO> childRegister(@RequestParam("anm-id") String anmIdentifier) {
		ChildRegistration childRegister = childRegisterService.getChildRegisterForProvider(anmIdentifier);
	...
	public ResponseEntity<List<CountServiceDTO>> getHouseHoldInformation(@RequestParam("anm-id") String provider,
	                                                                     @RequestParam("start-month") String startMonth,
	                                                                     @RequestParam("end-month") String endMonth,
	                                                                     @RequestParam("start-week") String startWeek,
	                                                                     @RequestParam("end-week") String endtWeek,
	                                                                     @RequestParam("type") String type) {
		dataCountService.getChildCountInformation(provider, startMonth, endMonth, startWeek, endtWeek, type);
		return new ResponseEntity<>(dataCountService.getChildCountInformation(provider, startMonth, endMonth, startWeek,
		    endtWeek, type), HttpStatus.OK);
	}
