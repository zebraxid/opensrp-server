...
public class ClientResourceTest extends BaseResourceTest {
	...
	public void shouldThrowExceptionWhileUpdateIfFistNameNotPresent() throws Exception {
		Client expectedNotUpdatedClient = new Client("1").withGender("male").withBirthdate(
		    new DateTime(0l, DateTimeZone.UTC), false);
	...
	public void shouldThrowExceptionWhileUpdateIfGenderNotPresent() throws Exception {
		Client expectedNotUpdatedClient = new Client("1").withFirstName("name").withBirthdate(
		    new DateTime(0l, DateTimeZone.UTC), false);
		addObjectToRepository(Collections.singletonList(expectedNotUpdatedClient), allClients);
		Client updatedClient = expectedNotUpdatedClient;
		updatedClient.setDeathdate(new DateTime(2l, DateTimeZone.UTC));
	...
	public void shouldSearchClient() throws Exception {
		Client expectedClient = (Client) new Client("1").withFirstName(name).withGender(male)
		        .withBirthdate(birthDate, false).withDeathdate(deathDate, true).withAddress(address);
		...
		String searchQuery = "search?name=" + name + "&gender=" + male + "&addressType=" + addressType + "&birthDate="
		        + birthDate.toLocalDate().toString() + "&deathDate=" + deathDate.toLocalDate().toString() + "&country="
		        + country + "&stateProvince=" + stateProvince + "&countryDistrict=" + countryDistrict + "&cityVillage="
		        + cityVillage + "&town=" + town + "&subDistrict=" + subDistrict;
	...
	public void shouldFailSearchClientWithInvalidQuery() throws Exception {
		...
		String searchQuery = "search?name=invalid" + name + "&gender=invalid" + male + "&addressType=" + addressType
	...
	public void shouldFailToSearchIfClientDoesntHaveAddressField() throws Exception {
		Client expectedClient = (Client) new Client("1").withFirstName(name).withGender(male)
		        .withBirthdate(birthDate, false).withDeathdate(deathDate, true);
		expectedClient.setDateCreated(new DateTime(0l, DateTimeZone.UTC));
	...
	public void shouldFilterData() throws Exception {
		...
		String searchQuery = "?q=name:" + name + "and gender:" + male + "and addressType:" + addressType + "and birthDate:"
		        + birthDate.toLocalDate().toString() + "and deathDate:" + deathDate.toLocalDate().toString()
		        + "and country:" + country + "and stateProvince:" + stateProvince + "and countryDistrict:" + countryDistrict
