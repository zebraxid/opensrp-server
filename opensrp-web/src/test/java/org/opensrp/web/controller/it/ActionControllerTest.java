...
public class ActionControllerTest extends BaseResourceTest {
	...
	public void shouldFetchNewActionsBasedOnAnmIdAndTimestamp() throws Exception {
		...
		data.put("key", "value");
		Action expectedAction = new Action("Case X", "ANM 1", ActionData.createAlert(mother.value(),
		    "Ante Natal Care - Normal", "ANC 1", normal, DateTime.now(), DateTime.now().plusDays(3)));
		...
		JsonNode actualObj = getCallAsJsonNode(url, "anmIdentifier=" + "ANM 1" + "&timeStamp="
		        + new DateTime().minusDays(1).getMillis(), status().isOk());
	...
	public void shouldFetchNewActionsBasedOnBaseEntityIdAndTimestamp() throws Exception {
		...
		JsonNode actualObj = getCallAsJsonNode(url, "baseEntityId=" + "Case X" + "&timeStamp="
		        + new DateTime().minusDays(1).getMillis(), status().isOk());
		org.opensrp.dto.Action actualActionDto = mapper.treeToValue(actualObj.get(0), org.opensrp.dto.Action.class);
	...
	public void shouldFetchNewActionsBasedOnTimestamp() throws Exception {
		...
		JsonNode actualObj = getCallAsJsonNode(url, "providerId=" + "ANM 1" + "&serverVersion="
		        + new DateTime().minusDays(1).getMillis(), status().isOk());
	...
	public void shouldReturnEmptyResponseForInvalidTimeStampWhileSync() throws Exception {
		...
		JsonNode actualObj = getCallAsJsonNode(url, "?providerId=" + "ANM 1" + "&serverVersion=" + "dsfs", status()
