...
public class AtomFeedTest extends TestResourceLoader {
    ...
    public void shouldReadEventsCreatedEvents() throws URISyntaxException {
		...
		PatientAtomfeed paf = new PatientAtomfeed(new AllMarkersInMemoryImpl(), new AllFailedEventsInMemoryImpl(), openmrsOpenmrsUrl, patientService, cs, errorTraceService);

		if(pushToOpenmrsForTest){
			paf.processEvents();
		}
        
		EncounterAtomfeed eaf = new EncounterAtomfeed(new AllMarkersInMemoryImpl(), new AllFailedEventsInMemoryImpl(), openmrsOpenmrsUrl, encounterService, es, errorTraceService);
		if(pushToOpenmrsForTest){
			eaf.processEvents();
		}
    }
