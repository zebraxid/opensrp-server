...
@Service
public class AtomFeedClientWrapper implements FeedClient{

	private static Logger logger = Logger.getLogger(AtomFeedClientWrapper.class);
    ...
    public AtomFeedClientWrapper(){
    	
    }
    AtomFeedClientWrapper(AllFeeds allFeeds, AllMarkers allMarkers, AllFailedEvents allFailedEvents, URI feedUri, EventWorker eventWorker) {
        this(allFeeds, allMarkers, allFailedEvents, new AtomFeedProperties(), null, feedUri, eventWorker);
    }

    public AtomFeedClientWrapper(AllFeeds allFeeds, AllMarkers allMarkers, AllFailedEvents allFailedEvents, AtomFeedProperties atomFeedProperties,
    ...
    public void processEvents() {
        ...
        try {
            ...
            logger.info("Last Read Entry:"+lastRead);
            final FeedEnumerator enumerator = new FeedEnumerator(allFeeds, lastRead);
            logger.info("Last Read enumerator:"+enumerator.toString());
            for (final Entry entry : enumerator) {
                ...
                try {
                    ...
                    logger.info(String.format("Processing event : %s", eventInProcess.getContent()));
