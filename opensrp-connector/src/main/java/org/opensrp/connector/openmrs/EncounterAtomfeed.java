...
public class EncounterAtomfeed extends OpenmrsService implements EventWorker, AtomfeedService {
	...
	public EncounterAtomfeed(AllMarkers allMarkers, AllFailedEvents allFailedEvents,
	    @Value("#{opensrp['openmrs.url']}") String baseUrl, ECEncounterService encounterService, EventService eventService)
	...
	public void process(Event event) {
		...
		try {
			JSONObject p = encounterService.getEncounterByUuid(
			    event.getContent().substring(event.getContent().lastIndexOf("/") + 1), true);
			System.out.println("Encounter:" + p);
