...
public class OpenmrsUserService extends OpenmrsService {
	...
	public boolean deleteAdminSession() {
		HttpResponse op = HttpUtil.delete(HttpUtil.removeEndingSlash(OPENMRS_BASE_URL) + "/" + AUTHENTICATION_URL, "", OPENMRS_USER, OPENMRS_PWD);
		return op.isSuccess();
	}
	...
	public User getUser(String username) throws JSONException {
		...
		User u = new User(obj.getString("uuid"), obj.getString("username"), null, preferredName, null, p.getString("display"), null, null);
	...
	public String getPersonUUIDByUser(String username) throws JSONException {
		HttpResponse op = HttpUtil.get(HttpUtil.removeEndingSlash(OPENMRS_BASE_URL) + "/" + USER_URL, PatientService.CUSTOM_PERSON_UUID_PARAM + "&username=" + username, OPENMRS_USER, OPENMRS_PWD);
		JSONObject res = new JSONObject(op.body());
