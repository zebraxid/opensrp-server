...
public class PatientService extends OpenmrsService {
	...
	public JSONObject createPerson(BaseEntity be) throws JSONException {
		...
		return new JSONObject(HttpUtil.post(getURL() + "/" + PERSON_URL, "", per.toString(), OPENMRS_USER, OPENMRS_PWD)
		        .body());
	}
		...
		for (Address ad : adl) {
			...
			if (ad.getAddressFields() != null) {
				...
				jao.put("countyDistrict",
				    ad.getAddressFieldMatchingRegex("(?i)(county_district|countyDistrict|COUNTY|DISTRICT)"));
	...
	public JSONObject createPatient(Client c) throws JSONException {
		...
		if (patientExist != null) {
			logger.info("person or patient already existis inside openmrs id:" + c.getBaseEntity().getId());
			return patientExist;
		}
		...
		if (c.getIdentifiers() != null) {
			for (Entry<String, String> id : c.getIdentifiers().entrySet()) {
				...
				if (patientExist != null) {
					logger.info("person or patient already existis inside openmrs with identifier:" + id.getValue());
					return patientExist;
				}
		...
		logger.info("going to create patient: " + p.toString());
		return new JSONObject(HttpUtil.post(getURL() + "/" + PATIENT_URL, "", p.toString(), OPENMRS_USER, OPENMRS_PWD)
		        .body());
	}
