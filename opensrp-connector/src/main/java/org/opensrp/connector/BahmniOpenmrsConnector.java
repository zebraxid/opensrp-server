...
public class BahmniOpenmrsConnector {
	...
	void fillAddressFields(FormField fl, String subform, FormSubmission fs, Map<String, Address> addresses)
	...
	public Client getClientFromFormSubmission(FormSubmission fs) throws ParseException {
		...
		Client c = new Client().withBaseEntity(
		    new BaseEntity(fs.entityId(), firstName, middleName, lastName, birthdate, deathdate, birthdateApprox,
		            deathdateApprox, gender, addresses, extractAttributes(fs))).withIdentifiers(extractIdentifiers(fs));
		return c;
	}
	...
	Map<String, Address> extractAddressesForSubform(Map<String, String> instance, String subform, FormSubmission fs)
	    throws ParseException {
		Map<String, Address> paddr = new HashMap<>();
	 ...
	 * @throws ParseException
	 */
	private Event getEventForSubform(FormSubmission fs, String subform, String eventType, Map<String, String> subformInstance)
	    throws ParseException {
		...
		List<String> a = new ArrayList<>();
		a.add("encounter_type");
		Event e = new Event().withBaseEntityId(subformInstance.get("id"))
		        .withEventDate(OpenmrsService.OPENMRS_DATE.parse(fs.getField(encounterDateField))).withEventType(eventType)
		        .withLocationId(fs.getField(encounterLocation)).withProviderId(fs.anmId());
		...
		for (Entry<String, String> fl : subformInstance.entrySet()) {
			...
			if (!StringUtils.isEmptyOrWhitespaceOnly(fl.getValue()) && att.size() > 0
			        && att.get("openmrs_entity").equalsIgnoreCase("concept")) {
				if (isMultiSelect(fl.getValue())) {
					String[] fieldValues = fl.getValue().split(" ");
					String val = "";
					for (String fieldVal : fieldValues) {
						val = formAttributeMapper.getInstanceAttributesForFormFieldAndValue(fl.getKey(), fieldVal, subform,
						    fs) + " ";
						...
						e.addObs(new Obs("concept", att.get("openmrs_entity_id"), att.get("openmrs_entity_parent"),
						        StringUtils.isEmptyOrWhitespaceOnly(val) ? fieldVal : val, null, fl.getKey()));
					}
					
				} else {
					String val = formAttributeMapper.getInstanceAttributesForFormFieldAndValue(fl.getKey(), fl.getValue(),
					    subform, fs);
					...
					e.addObs(new Obs("concept", att.get("openmrs_entity_id"), att.get("openmrs_entity_parent"), StringUtils
					        .isEmptyOrWhitespaceOnly(val) ? fl.getValue() : val, null, fl.getKey()));
				}
			}
		}
		return e;
	 ...
	 * beneficiary (excluding main beneficiary). The dependent entities are specified via subforms
	 * (repeat groups) in xls forms.
	 * 
	 * @param fs
	 * @return The clients and events Map with id of dependent entity as key. Each entry in Map
	 *         contains an internal map that holds Client and Event info as "client" and "event"
	 *         respectively for that dependent entity (whose id is the key of main Map). Ex:
	 *         {222222-55d555-ffffff-232323-ffffff: {client: ClientObjForGivenID, event:
	 *         EventObjForGivenIDAndForm}}, {339393-545445-ffdddd-333333-ffffff: {client:
	 *         ClientObjForGivenID, event: EventObjForGivenIDAndForm}},
		...
		for (SubFormData sbf : fs.subForms()) {
			...
			if (att.size() > 0 && att.get("openmrs_entity").equalsIgnoreCase("person")) {
				for (Map<String, String> sfdata : sbf.instances()) {
					...
					if (StringUtils.isEmptyOrWhitespaceOnly(firstName) && idents.size() < 2
					        && !sbf.name().equals("child_registration")) { //we need to ignore uuid of entity
					...
					Client c = new Client().withBaseEntity(
					    new BaseEntity(sfdata.get("id"), firstName, middleName, lastName, birthdate, deathdate,
					            birthdateApprox, deathdateApprox, gender, addresses, extractAttributes(sfdata, sbf.name(),
					                fs))).withIdentifiers(idents);
	...
	public Event getEventFromFormSubmission(FormSubmission fs) throws ParseException {
		...
		for (FormField fl : fs.instance().form().fields()) {
			...
			if (!StringUtils.isEmptyOrWhitespaceOnly(fl.value()) && att.size() > 0
			        && att.get("openmrs_entity").equalsIgnoreCase("concept")) {
				if (isMultiSelect(fl.value())) {
					...
					for (String fieldVal : fieldValues) {
						...
						e.addObs(new Obs("concept", att.get("openmrs_entity_id"), att.get("openmrs_entity_parent"),
						        StringUtils.isEmptyOrWhitespaceOnly(val) ? fieldVal : val, null, fl.name()));
					}
				} else {
					String val = formAttributeMapper.getInstanceAttributesForFormFieldAndValue(fl.name(), fl.value(), null,
					    fs);
					e.addObs(new Obs("concept", att.get("openmrs_entity_id"), att.get("openmrs_entity_parent"), StringUtils
					        .isEmptyOrWhitespaceOnly(val) ? fl.value() : val, null, fl.name()));
				}
			}
		}
		return e;
