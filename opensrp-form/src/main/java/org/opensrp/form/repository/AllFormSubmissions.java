...
public class AllFormSubmissions extends MotechBaseRepository<FormSubmission> {
	...
	public List<FormSubmission> findByServerVersion(long serverVersion) {
		...
		return db.queryView(
		    createQuery("formSubmission_by_server_version").startKey(startKey).endKey(endKey).includeDocs(true),
	...
	public List<FormSubmission> allFormSubmissions(long serverVersion, Integer batchSize) {
		ComplexKey startKey = ComplexKey.of(serverVersion + 1);
		ComplexKey endKey = ComplexKey.of(Long.MAX_VALUE);
		ViewQuery query = createQuery("formSubmission_by_server_version").startKey(startKey).endKey(endKey)
		        .includeDocs(true);
	...
	@View(name = "formSubmission_by_anm", map = "function(doc) { if (doc.type === 'FormSubmission' && doc.anmId) { emit(doc.anmId, null); } }")
	public List<FormSubmission> findByANMID(String anmId) {
		
		return db.queryView(createQuery("formSubmission_by_anm").key(anmId).includeDocs(true), FormSubmission.class);
	}
	...
	public List<FormSubmission> findByANMIDAndServerVersion(String anmId, long version, Integer batchSize) {
		...
		ViewQuery query = createQuery("formSubmission_by_anm_and_server_version").startKey(startKey).endKey(endKey)
		        .includeDocs(true);
		if (batchSize != null) {
			query.limit(batchSize);
		}
	...
	@View(name = "formSubmission_by_anm_and_usertype_and_server_version", map = "function(doc) {if(doc.formInstance.form.fields.length > 0 && doc.type==='FormSubmission') {for(var field in doc.formInstance.form.fields) {if(doc.formInstance.form.fields[field].name ==='user_type'){emit([doc.anmId,doc.formInstance.form.fields[field].value, doc.serverVersion], null);}}}}")
	public List<FormSubmission> findByANMIDAndUserTypeAndServerVersion(String anmId, String userType, long version,
	                                                                   Integer batchSize) {
		...
		ViewQuery query = createQuery("formSubmission_by_anm_and_usertype_and_server_version").startKey(startKey)
	...
	@View(name = "formSubmission_by_both_user_fwa_after_90", map = "function(doc) { if(doc.formInstance.form.fields.length > 0 && doc.type==='FormSubmission') { for(var field in doc.formInstance.form.fields) { if(doc.formInstance.form.fields[field].name ==='user_type'){ if (doc.formInstance.form.fields[field].value==='FD'){ emit([doc.anmId, doc.serverVersion], null); } else if (doc.formInstance.form.fields[field].value==='FWA' && doc.serverVersion > 1456358400000){ emit([doc.anmId, doc.serverVersion], null); } } } } }")
	public List<FormSubmission> findBothUserSubmissionFWAAfter90(String anmId, long version, Integer batchSize) {
		
		ComplexKey startKey = ComplexKey.of(anmId, version + 1);
		ComplexKey endKey = ComplexKey.of(anmId, Long.MAX_VALUE);
		
		ViewQuery query = createQuery("formSubmission_by_both_user_fwa_after_90").startKey(startKey).endKey(endKey)
		        .includeDocs(true);
		if (batchSize != null) {
			query.limit(batchSize);
		}
	...
	public List<FormSubmission> getByEntityId(String entityId) {
		...
		return db.queryView(createQuery("form_by_entity").key(entityId).includeDocs(true), FormSubmission.class);
	}
	
	@View(name = "form_by_provider_form_definition", map = "function(doc) { if (doc.type === 'FormSubmission' && doc.entityId && doc.formDataDefinitionVersion=='9001' && doc.formName=='new_household_registration') { emit(doc.anmId,null ); } }")
	public List<FormSubmission> getByProviderAndFormDefinition(String provider) {
		
		return db.queryView(createQuery("form_by_provider_form_definition").key(provider).includeDocs(true),
		    FormSubmission.class);
	}
