...
public class FormSubmissionService {
	...
	public FormSubmissionService(AllFormSubmissions allFormSubmissions,
	    @Value("#{opensrp['mcare2.user.type']}") String userType) {
		this.allFormSubmissions = allFormSubmissions;
		this.userType = userType;
	}
	
	public List<FormSubmissionDTO> fetch(long formFetchToken) {
		return with(allFormSubmissions.findByServerVersion(formFetchToken)).convert(
	...
	public void submit(List<FormSubmissionDTO> formSubmissionsDTO) {
		List<FormSubmission> formSubmissions = with(formSubmissionsDTO).convert(
		...
		for (FormSubmission submission : formSubmissions) {
			...
			logger.info(format("Saving form {0} with instance Id: {1} and for entity Id: {2}", submission.formName(),
