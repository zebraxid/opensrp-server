...
/*index = "function(doc) { " + 
    ...
    "}")
*/
@FullText({ @Index(name = "by_provider", index = "function(rec) {" + " var doc=new Document();"
        ...
        + " doc.add(rec.type,{\"field\":\"type\", \"store\":\"yes\"});" + " return doc;" + "}") })
@Repository
public class LuceneChildRepository extends CouchDbRepositorySupportWithLucene<Child> {
	
	@Autowired
	public LuceneChildRepository(@Qualifier(AllConstants.OPENSRP_DATABASE_LUCENE_CONNECTOR) LuceneAwareCouchDbConnector db) {
