...
public class LuceneElcoRepositoryTest {
    ...
    public void testInit() throws ParseException { 
    	...
    	Date day= null;
		try {
			day = dateFormat.parse(dateFormat.format(today));			
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("timestamp with previous way- " + day.getTime());
		System.out.println("timestamp with new way- " + DateUtil.getTimestampToday());*/
    	...
    	String makeQueryString ="type:Household" + " AND "+ 
    			"SUBMISSIONDATE:[1465840800000 TO 1465840800000]" ;
        ...
        for(int i = 0 ; i < boundaries.weekBoundariesAsTimeStamp.size(); i++){
        	System.out.println(boundaries.weekBoundariesAsTimeStamp.get(i) + " -this timestamp should refer to- " 
        					+ boundaries.weekBoundariesAsString.get(i) + " -and referring to- " 
        			+ new Date(boundaries.weekBoundariesAsTimeStamp.get(i)).toString());
        }
        ...
        System.out.println(result.getRows().size() + " -count with getRows().size() for today");
        System.out.println(result.getTotalRows() + " -count with getTotalRows() for today");*/
